    ...
    messages.forEach(paymentRequestAvroModel -> {
      try {
        ...
        if (sqlException != null
          ...
          // NO-OP for violation exception
          log.error(
              "Caught UniqueConstraintException with SQL state: {} in {} for order id: {}",
              sqlException.getSQLState(),
              this.getClass().getSimpleName(),
              paymentRequestAvroModel.getOrderId()
          );
        } else {
          throw new PaymentApplicationServiceException("Throwing " + e.getClass().getSimpleName() + " in " + this
              .getClass().getSimpleName() + ": " + e.getMessage(), sqlException);
        }
